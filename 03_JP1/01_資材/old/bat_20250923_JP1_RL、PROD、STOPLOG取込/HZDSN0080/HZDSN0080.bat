...
rem
rem 引数:
rem   - なし（自フォルダの HZDSN0080.ini を使用）
rem
rem ログ:
rem   - exec\log\HZDSN0080\HZDSN0080_yyyyMMdd_HHmmss_{out|err}.log
rem     ※出力は共通バッチ側で実施
rem
rem 戻り値:
...
rem
rem 注意:
rem   - 本ファイルの配置: exec\bin\bat\HZDSN0080\HZDSN0080.bat
...
call "%HERE%..\..\common\run_winscp.bat" "%HERE%HZDSN0080.ini" "HZDSN0080"
exit /b %ERRORLEVEL%
